...
const pill=(bg,color,border)=>({background:bg,color,border:`1px solid ${border||bg}`,borderRadius:20,padding:"2px 10px",fontSize:11,fontWeight:700,fontFamily:"inherit",whiteSpace:"nowrap"});
...
function EntradaModal({ins,onSave,onClose}){
  ...
  return(
    <div style={{position:"fixed",inset:0,background:"rgba(0,0,0,0.4)",zIndex:300,display:"flex",alignItems:"flex-end",backdropFilter:"blur(2px)"}} onClick={onClose}>
      <div style={{background:C.surface,borderRadius:"20px 20px 0 0",width:"100%",maxWidth:480,margin:"0 auto",padding:"20px 20px 36px",maxHeight:"85vh",overflowY:"auto"}} onClick={e=>e.stopPropagation()}>
        ...
        <div style={{background:C.primaryLight,border:`1px solid ${C.primaryMid}`,borderRadius:12,padding:"12px 14px",marginBottom:16}}>
          ...
          {hasConv(ins)&&<p style={{fontSize:13,color:C.primary,margin:"3px 0 0",fontWeight:700}}>= {(ins.estoque/ins.itens_por_embalagem).toFixed(1)} {ins.unidade_compra} ({ins.itens_por_embalagem} un/{ins.unidade_compra})</p>}
        </div>
        {hasConv(ins)&&(
          <>
            <label style={lbl}>Informar por</label>
            <div style={{display:"flex",gap:8,marginBottom:16}}>
              {[["emb",ins.unidade_compra],["un","Unidades avulsas"]].map(([v,l])=>(
                <button key={v} onClick={()=>setTipo(v)} style={{flex:1,padding:"10px",borderRadius:10,border:`2px solid ${tipo===v?C.teal:C.border}`,background:tipo===v?C.tealLight:C.surface,color:tipo===v?C.teal:C.text,fontWeight:700,fontSize:13,cursor:"pointer",fontFamily:"inherit"}}>{l}</button>
              ))}
            </div>
          </>
        )}
        ...
        {qtd&&parseFloat(qtd)>0&&(
          <div style={{background:C.greenLight,border:`1px solid ${C.greenMid}`,borderRadius:12,padding:"12px 14px",marginTop:12}}>
            ...
            {tipo==="emb"?<p style={{fontSize:15,fontWeight:700,color:C.green,margin:0}}>{qtd} {ins.unidade_compra} = <strong>{calc} unidades</strong></p>:<p style={{fontSize:15,fontWeight:700,color:C.green,margin:0}}>{calc} unidades</p>}
        ...
        <input style={inp} placeholder="Ex: Recebido do hospital em 12/05" value={obs} onChange={e=>setObs(e.target.value)}/>
...
function Login({onLogin}){
  ...
  const [reg,setReg]=useState(false);const [loading,setLoading]=useState(false);const [msg,setMsg]=useState("");
  const handle=async()=>{
    ...
    try{
      if(reg){
        ...
        if(e)throw e;
        if(data.user){await supabase.from("profiles").insert({id:data.user.id,name,email,role:"caregiver"});await supabase.from("patient_caregivers").insert({patient_id:PATIENT_ID,profile_id:data.user.id});}
        setMsg("Conta criada! Verifique seu e-mail.");
      }else{
        const{error:e}=await supabase.auth.signInWithPassword({email,password:pw});
        if(e)throw e;onLogin();
      }
  ...
  return(
    <div style={{minHeight:"100vh",background:"linear-gradient(135deg,#1D4ED8,#3B82F6)",display:"flex",alignItems:"center",justifyContent:"center",padding:20,fontFamily:"'Nunito','Segoe UI',sans-serif"}}>
      <div style={{background:"#fff",borderRadius:20,padding:"32px 24px",width:"100%",maxWidth:400,boxShadow:"0 20px 60px rgba(0,0,0,0.2)"}}>
        ...
        {reg&&<><label style={lbl}>Nome</label><input style={inp} placeholder="Seu nome" value={name} onChange={e=>setName(e.target.value)}/></>}
        <label style={lbl}>E-mail</label><input style={inp} type="email" placeholder="seu@email.com" value={email} onChange={e=>setEmail(e.target.value)}/>
        <label style={lbl}>Senha</label><input style={inp} type="password" placeholder="Senha" value={pw} onChange={e=>setPw(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()}/>
        ...
        {msg&&<p style={{fontSize:13,color:msg.includes("criada")?C.green:C.red,marginTop:12,textAlign:"center",fontWeight:600}}>{msg}</p>}
...
export default function App(){
  ...
  const toast2=(msg,tipo="ok")=>{setToast({msg,tipo});setTimeout(()=>setToast(null),3200);};
  ...
  const loadAll=async()=>{
    setLoading(true);
    const[{data:sup},{data:mov}]=await Promise.all([
      supabase.from("supplies").select("*, supply_lots(*)").eq("patient_id",PATIENT_ID).eq("active",true).order("nome"),
      supabase.from("stock_movements").select("*, profiles(name)").eq("patient_id",PATIENT_ID).order("created_at",{ascending:false}).limit(50),
    ]);
    if(sup)setInsumos(sup.map(i=>({...i,validades:i.supply_lots||[]})));
    if(mov)setHistorico(mov);
    setLoading(false);
  };

  useEffect(()=>{
    if(!session)return;
    const ch=supabase.channel("rt").on("postgres_changes",{event:"*",schema:"public",table:"supplies",filter:`patient_id=eq.${PATIENT_ID}`},()=>loadAll()).on("postgres_changes",{event:"*",schema:"public",table:"stock_movements",filter:`patient_id=eq.${PATIENT_ID}`},()=>loadAll()).subscribe();
  ...
  const handleEntrada=async(unidades,obs,emb)=>{
    ...
    await supabase.from("stock_movements").insert({supply_id:entModal.id,patient_id:PATIENT_ID,profile_id:session.user.id,tipo:"entrada",quantidade:unidades,observacao:emb?`Entrada: ${emb} ${entModal.unidade_compra} = ${unidades} un${obs?". "+obs:""}`:(`Entrada: ${unidades} un${obs?". "+obs:""}`)});
    toast2(`+${unidades} unidades adicionadas!`);setEntModal(null);
  };

  const handleBaixa=async()=>{
    const q=parseInt(baixaQtd);
    if(isNaN(q)||q<=0)return toast2("Quantidade invalida","erro");
    if(q>sel.estoque)return toast2("Estoque insuficiente","erro");
    const c=[...(sel.consumo_mensal||[0,0,0,0])];c[c.length-1]=(c[c.length-1]||0)+q;
    ...
    toast2(`Baixa de ${q} un registrada!`);setModal(null);setBaixaQtd(1);
  };

  const handleSalvar=async()=>{
    if(!form.nome||!form.categoria)return toast2("Preencha nome e categoria","erro");
    const ipe=parseInt(form.itens_por_embalagem)||1;
    const tc=form.unidade_compra&&form.unidade_compra!==""&&ipe>1;
    const payload={patient_id:PATIENT_ID,codigo:form.codigo||"",nome:form.nome,categoria:form.categoria,unidade:form.unidade||"Unid",estoque:parseInt(form.estoque)||0,minimo:parseInt(form.minimo)||5,unidade_compra:tc?form.unidade_compra:null,itens_por_embalagem:tc?ipe:1};
    if(modal==="novo"){await supabase.from("supplies").insert({...payload,consumo_mensal:[0,0,0,0]});toast2("Insumo cadastrado!");}
    else{await supabase.from("supplies").update(payload).eq("id",sel.id);toast2("Insumo atualizado!");}
    setModal(null);setForm({});
  };

  const handleLote=async()=>{
    if(!form.novoLote||!form.novaValidade)return toast2("Preencha lote e validade","erro");
    ...
    toast2("Lote adicionado!");setForm({});
  };

  const openM=(t,i=null)=>{setSel(i);setForm(i?{...i,itens_por_embalagem:i.itens_por_embalagem||1}:{categoria:"Materiais",unidade:"Unid",itens_por_embalagem:1});setBaixaQtd(1);setModal(t);};
  const togS=(i)=>setSolic(p=>{const e=p.find(x=>x.id===i.id);if(e)return p.filter(x=>x.id!==i.id);return[...p,{...i,qtdS:Math.max(1,i.minimo-i.estoque)}];});
  const updS=(id,q)=>setSolic(p=>p.map(i=>i.id===id?{...i,qtdS:parseInt(q)||1}:i));

  const enviarSolic=()=>{
    if(!emailH)return toast2("Configure o e-mail","erro");
    if(solic.length===0)return toast2("Selecione um item","erro");
    ...
    toast2("E-mail preparado!");
  };

  const exportPDF=()=>{
    const lista=sug.filter(i=>Math.max(0,i.sug-i.estoque)>0);
    const rows=lista.map(i=>{const pu=Math.max(0,i.sug-i.estoque);const pc=hasConv(i)?Math.ceil(pu/i.itens_por_embalagem):null;const ps=pc?`${pc} ${i.unidade_compra} (${pu} un)`:`${pu} un`;return`<tr><td>${i.nome}</td><td style="text-align:center">${i.codigo||"-"}</td><td style="text-align:center">${lblEst(i)}</td><td style="text-align:center;font-weight:bold;color:#2563EB">${ps}</td></tr>`;}).join("");
    ...
    const a=document.createElement("a");a.href=URL.createObjectURL(new Blob([html],{type:"text/html"}));a.download=`pedido_${fmt(new Date()).replace(/\//g,"-")}.html`;a.click();
    toast2("Lista exportada!");
  };

  const filt=useMemo(()=>insumos.filter(i=>{const ok=i.nome.toLowerCase().includes(busca.toLowerCase())||i.categoria.toLowerCase().includes(busca.toLowerCase())||(i.codigo&&i.codigo.includes(busca));return ok&&(cat==="Todas"||i.categoria===cat);}),[insumos,busca,cat]);
  const alerts=useMemo(()=>{const r=[];insumos.forEach(i=>{if(i.estoque<=i.minimo)r.push({t:"est",i});i.validades?.forEach(v=>{const d=diffDays(v.validade,today);if(d<=30)r.push({t:"val",i,d});});});return r;},[insumos]);
  ...
  const s={
    ...
    ov:{position:"fixed",inset:0,background:"rgba(0,0,0,0.35)",zIndex:200,display:"flex",alignItems:"flex-end",backdropFilter:"blur(2px)"},
  ...
  return(
    <div style={s.app}>
      ...
      <nav style={s.nav}>
        {[["estoque","Estoque"],["alertas","Alertas"],["sugestoes","Pedido"],["solicitar","Solicitar"],["relatorios","Relatorios"]].map(([id,label])=>(
          <button key={id} style={s.nb(aba===id)} onClick={()=>setAba(id)}>{label}{id==="alertas"&&alerts.length>0?` (${alerts.length})`:""}</button>
        ))}
      </nav>
      ...
      <div style={s.con}>
              ...
              <div style={{background:C.surfaceAlt,borderRadius:10,padding:"10px 12px",marginBottom:10}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    {hasConv(i)&&<p style={{fontSize:11,color:C.teal,margin:0,fontWeight:600}}>{(i.minimo/i.itens_por_embalagem).toFixed(1)} {i.unidade_compra}</p>}
                  </div>
                </div>
                <div style={{height:6,background:C.border,borderRadius:4,overflow:"hidden",marginTop:8}}><div style={{height:"100%",width:`${pct(i.estoque,i.minimo)}%`,background:pcolor(i.estoque,i.minimo),borderRadius:4,transition:"width 0.5s"}}/></div>
              </div>
        ...
        {!loading&&aba==="alertas"&&<>
          <div style={s.sg}>
            {[[C.red,insumos.filter(i=>i.estoque===0).length,"Esgotados"],[C.orange,insumos.filter(i=>i.estoque>0&&i.estoque<=i.minimo).length,"Criticos"],[C.yellow,insumos.filter(i=>i.validades?.some(v=>diffDays(v.validade,today)<=30)).length,"Vencem 30d"],[C.green,insumos.filter(i=>i.estoque>i.minimo).length,"Em Dia"]].map(([c,n,l])=>(
              <div key={l} style={s.sc(c)}><div style={{fontSize:30,fontWeight:900,color:c}}>{n}</div><div style={{fontSize:12,color:C.textSub,marginTop:2,fontWeight:700}}>{l}</div></div>
            ))}
          </div>
          ...
          {alerts.filter(a=>a.t==="est").length>0&&<>
              ...
              <div key={idx} style={{...s.card,borderLeft:`4px solid ${a.i.estoque===0?C.red:C.orange}`}}>
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{textAlign:"right"}}>
                    ...
                    {hasConv(a.i)&&<p style={{fontSize:11,color:C.teal,margin:0,fontWeight:700}}>{(a.i.estoque/a.i.itens_por_embalagem).toFixed(1)} {a.i.unidade_compra}</p>}
          ...
          {alerts.filter(a=>a.t==="val").length>0&&<>
        ...
        {!loading&&aba==="sugestoes"&&<>
          <div style={{...s.card,background:C.primaryLight,borderColor:C.primaryMid,marginBottom:16}}>
            <p style={{fontSize:13,color:C.primary,margin:0,fontWeight:600}}>Sugestao baseada no consumo medio + 20%. Quantidades mostradas em unidade de compra quando aplicavel.</p>
          </div>
        ...
        {!loading&&aba==="solicitar"&&<>
          <div style={{...s.card,background:emailCfg?C.surface:C.primaryLight,borderColor:C.primaryMid,marginBottom:14}}>
            ...
            {emailCfg&&<><label style={{...s.lbl,marginTop:10}}>E-mail</label><input style={s.inp} type="email" value={emailH} onChange={e=>setEmailH(e.target.value)} placeholder="insumos@hospital.com.br"/></>}
          ...
          <div style={{display:"flex",flexDirection:"column",gap:8,marginBottom:16}}>
            {[["urgencia","Urgente","Item esgotado"],["reposicao","Reposicao","Fora da programacao"],["complementar","Complementar","Item adicional"]].map(([v,l,d])=>(
              <div key={v} onClick={()=>setMotivoS(v)} style={{...s.card,borderColor:motivoS===v?C.primary:C.border,borderWidth:motivoS===v?2:1,cursor:"pointer",background:motivoS===v?C.primaryLight:C.surface,padding:"12px 14px"}}>
                <div style={{display:"flex",alignItems:"center",gap:10}}>
                  <div style={{width:20,height:20,borderRadius:"50%",border:`2px solid ${motivoS===v?C.primary:C.border}`,background:motivoS===v?C.primary:"transparent",display:"flex",alignItems:"center",justifyContent:"center"}}>{motivoS===v&&<div style={{width:8,height:8,borderRadius:"50%",background:"#fff"}}/>}</div>
                  <div><p style={{fontSize:14,fontWeight:800,margin:0,color:motivoS===v?C.primary:C.text}}>{l}</p><p style={{fontSize:11,color:C.textMuted,margin:"2px 0 0"}}>{d}</p></div>
                </div>
              </div>
            ))}
          </div>
          {insumos.filter(i=>i.estoque<=i.minimo).length>0&&<button style={{...s.btnP,width:"100%",marginBottom:12,background:C.orange}} onClick={()=>{const cr=insumos.filter(i=>i.estoque<=i.minimo);const nv=cr.filter(c=>!solic.find(s=>s.id===c.id));setSolic(p=>[...p,...nv.map(i=>({...i,qtdS:Math.max(1,i.minimo-i.estoque+Math.ceil(i.minimo*0.2))}))]); toast2(`${nv.length} itens adicionados!`);}}>Adicionar {insumos.filter(i=>i.estoque<=i.minimo).length} critico(s)</button>}
          ...
          {insumos.filter(i=>!buscaS||i.nome.toLowerCase().includes(buscaS.toLowerCase())).map(i=>{const sl=!!solic.find(s=>s.id===i.id);return(
            ...
            {historico.slice(0,40).map((h,idx)=>(
              <div key={idx} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 0",borderBottom:`1px solid ${C.border}`}}>
                <div style={{flex:1,marginRight:8}}>
                  ...
                  <p style={{fontSize:11,color:C.textMuted,fontWeight:600}}>{new Date(h.created_at).toLocaleDateString("pt-BR")} {new Date(h.created_at).toLocaleTimeString("pt-BR",{hour:"2-digit",minute:"2-digit"})} - {h.profiles?.name||""}</p>
        ...
        <div style={s.ov} onClick={()=>setModal(null)}>
          <div style={s.mb} onClick={e=>e.stopPropagation()}>
            ...
            {modal==="baixa"&&sel&&<>
              ...
              {hasConv(sel)&&parseInt(baixaQtd)>0&&<p style={{fontSize:12,color:C.teal,marginTop:6,fontWeight:600}}>= {(parseInt(baixaQtd)/sel.itens_por_embalagem).toFixed(2)} {sel.unidade_compra}</p>}
            ...
            {(modal==="novo"||modal==="editar")&&<>
              ...
              <div style={{display:"flex",flexDirection:"column",gap:14}}>
                ...
                <div style={{background:C.tealLight,border:`1px solid ${C.tealMid}`,borderRadius:12,padding:"14px"}}>
                ...
                <div style={s.r2}>
                  <div>
                    <label style={s.lbl}>Estoque atual (un)</label>
                    <input style={s.inp} type="number" min="0" value={form.estoque||""} onChange={e=>setForm(f=>({...f,estoque:e.target.value}))} placeholder="0"/>
                    {form.unidade_compra&&form.itens_por_embalagem>1&&form.estoque>0&&<p style={{fontSize:11,color:C.teal,margin:"4px 0 0",fontWeight:600}}>= {(parseInt(form.estoque)/parseInt(form.itens_por_embalagem)).toFixed(1)} {form.unidade_compra}</p>}
                  </div>
                  <div>
                    <label style={s.lbl}>Minimo (un)</label>
                    <input style={s.inp} type="number" min="0" value={form.minimo||""} onChange={e=>setForm(f=>({...f,minimo:e.target.value}))} placeholder="5"/>
                    {form.unidade_compra&&form.itens_por_embalagem>1&&form.minimo>0&&<p style={{fontSize:11,color:C.teal,margin:"4px 0 0",fontWeight:600}}>= {(parseInt(form.minimo)/parseInt(form.itens_por_embalagem)).toFixed(1)} {form.unidade_compra}</p>}
                  </div>
                </div>
              </div>
              <div style={{display:"flex",gap:8,marginTop:20}}>
            ...
            {modal==="validade"&&sel&&<>
              ...
              {sel.validades?.map((v,idx)=>{const d=diffDays(v.validade,today);return(
